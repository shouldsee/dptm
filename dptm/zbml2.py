...
class ZBML2(nn.Module):
  '''
  Interpolate between two examples
  '''
  def __init__(self, 
      params,
      device=None,
    ...
    #   self.b_scale_out   = nn.Parameter(torch.tensor(-5.,device=device))
    ...
    #   self.whz           = nn.Parameter(nn.Linear(Z,FV*T).weight.reshape(T,Z,FV).to(device))
      self.whz           = nn.Parameter(nn.Linear(Z,FV).weight.reshape(Z,FV).to(device))
  ...
  def sample(self, 
    ...
    for t in range(T):
        ...
        xbz[:,t:t+1]= xbzt
    A = self.params.A
    xa = torch.randint(0,self.params.A,size=(B,1),device=device)/(A-1)
  ...
  def loss_joint_lp(self, dat):
    ...
    xa = torch.zeros((B,M),device=device).long()
    # wzl = self.wzl
    # nstep = 1
    ...
    # idx_bmk = torch.arange(T,device=device)[None,None,None].expand(B,M,K,T)
    ...
    for _ in range(nstep):
        for t in range(T*2):
            ...
            if is_update_z:
                ...
                ### xa is index
                xka = xa[:,:,None,None]/(A-1)
            else:
                xkz = xmz[:,:,None,:]
                ### (1,1,A,1)
                xka = torch.arange(A,device=device)[None,None,:,None]/(A-1)
                pass
            ...
            if 1:
                ...
                loc_pred = (nl+bias[None,None,None, :,])            
                ### (B,M,K,L)
                mod_pred = torch.distributions.normal.Normal(loc=loc_pred,scale=b_scale_out.exp())
                ## node_bx
                lp_external = mod_pred.log_prob(node_bx[:,None,None,:]).sum(-1)
            lp_external

            if is_update_z:
                #### fitting internal preference
                ## (B,M,K)
                ...
                ### (B,M,1,T)
                xkz_parent = torch.cat([torch.zeros((B,M,1,1),device=device).long(),
                    xkz[:,:,:,:-1]
                ],dim=-1)
                
                ### (T,Z1,Z2)
                ### (B,M,1,T,Z2)
                lp_prior = lp_prior_0[idx_bmkt[:,:,0:1], xkz_parent, :]
                prior = torch.distributions.Categorical(logits=lp_prior)                
                ### no difference between internal prior during updating
                lp_internal = prior.log_prob(xkz).sum(-1)
                # lp_internal = 0.
            lp_internal = lp_internal + math.log(1./A)
