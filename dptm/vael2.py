...
class VAEL2(nn.Module):
  ...
  def __init__(self, 
      ...
      self.w_enc_2 = nn.Linear(E,E).to(device)
      self.w_dec_2 = nn.Linear(E,E).to(device)
  ...
  def sample(self, 
    ...
    # z_noise = z_noise + self.w_dec_2(z_noise).sigmoid()
    z_noise = self.w_dec_2(z_noise)
  ...
  def loss_joint_lp(self, dat):
    ...
    loc = w_enc(img_flat)
    # loc = loc + self.w_enc_2(loc).sigmoid()
    loc = self.w_enc_2(loc)
    ...
    # lse = (lp_internal.sum(-1)+lp_external.sum(-1)).mean(0) 
    # - math.log(NS)
    loss = - lse.mean(0)
