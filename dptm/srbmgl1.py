...
class SRBMGL1(nn.Module):
  ...
  def __init__(self, 
    ...
    #   self.w_enc = nn.Linear(FV,E).to(device)
      ...
      self.b_loc_prior = nn.Parameter( nn.Linear(E,1).weight.reshape((E,)).to(device))
  ...
  def loss_joint_lp(self, dat):
    ...
    for i in range(inner_nstep):
        ...
        node_h = arr_random_update(node_h, (dh1).sigmoid(), p_h_update)    
    ...
    post = torch.distributions.Bernoulli(node_h)
